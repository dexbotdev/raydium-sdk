...
export class MarketV2OPT extends Base {
  static async makeCreateMarketInstructionSimple<T extends TxVersion>({
    ...
    const baseLotSize = new BN(Math.round(
      10 ** baseInfo.decimals * Math.pow(10, -1 * baseInfo.decimals))
    );
    const quoteLotSize = new BN(Math.round(
      10 ** quoteInfo.decimals *
      Math.pow(10, -1 * baseInfo.decimals) *
      Math.pow(10, -1 * -2)
    ))
